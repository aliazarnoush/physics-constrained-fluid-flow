...
def run_uncertainty_quantification(model_path, alpha_mean=0.5, alpha_std=0.1, 
    ...
    def visualize_uncertainty(output_idx, output_name):
        ...
        plt.tight_layout()
        plt.savefig(f'{save_dir}/uncertainty_{output_name.lower()}.png', dpi=300)
        plt.close()
        
        print(f"Saved uncertainty plot for {output_name} to {save_dir}/uncertainty_{output_name.lower()}.png")
    ...
    # Function to apply model with a specific alpha
    def predict_with_alpha(alpha):
        predictions = jax.vmap(lambda x: model.apply(params, x, alpha))(points)
